...
def hours_vs_date_given_employee(present_qs, time_qs, admin=True):
    ...
    for obj in qs:
        ...
        print(time_qs.filter(date=date))
        if (len(times_out) > 0):
            print(times_out.last().time)
...
def dashboard2(request):
    if (request.user.username == 'admin'):
        ...
        return render(request, 'recognition/EmployeesLayout.html')
    ...
    while (True):
        ...
        for face in faces:
            ...
            if (pred != [-1]):
                ...
                cv2.putText(frame, str(person_name) + str(prob) +"Attendence Marked", (x+6,
        ...
        for face in faces:
            ...
            if (pred != [-1]):
                ...
                if count[pred] == 4 and (time.time()-start[pred]) > 1.5:
                    ...
                    count[pred] = count.get(pred, 0) + 1
                    print(pred, present[pred], count[pred])
                cv2.putText(frame, str(person_name) + str(prob) + "Attendence Marked Thanks", (x+6,
                            y+h-6), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
